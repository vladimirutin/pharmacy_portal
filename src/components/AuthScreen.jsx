...
export default function AuthScreen({ onLogin }) {
  ...
  return (
    <div className="flex min-h-screen min-h-[100dvh] w-full bg-[#0B0F19] overflow-y-auto relative">
      ...
      {showForgotPassword && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-lg p-4 animate-modal-fade">
          <div className="auth-card bg-[#0d1220] w-full max-w-sm border border-emerald-500/20 overflow-hidden" style={{ borderRadius: 28, boxShadow: '0 0 80px rgba(16, 185, 129, 0.08)' }}>
            <div className="p-8 text-center relative">
              <div className="absolute inset-0 bg-gradient-to-r from-emerald-600/15 to-teal-600/15 opacity-50" />
              <div className="relative z-10">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-5 mx-auto bg-gradient-to-br from-emerald-500 to-teal-600 shadow-[0_8px_32px_rgba(16,185,129,0.35)]">
                  <span className="text-3xl">🔑</span>
                </div>
                <h3 className="text-xl font-black text-white mb-2">Forgot Password?</h3>
                <p className="text-sm text-slate-400 leading-relaxed mb-6">
                  For security reasons, please contact our customer service department at <span className="text-emerald-400 font-bold whitespace-nowrap">09273523900</span> for further assistance.
                </p>
                <button type="button" onClick={() => setShowForgotPassword(false)} className="w-full justify-center py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-[0_8px_32px_rgba(16,185,129,0.25)] transition-all">
                  Got it
                </button>
              </div>
            </div>
